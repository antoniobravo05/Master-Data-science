...
#para usar spark en el sistema nativo hay que tener instalado java.
#para instalar spark basta con instalar el paquete sparklyr, ejecutandolo desde R. El solo lo instalará.
#pero no nos funciona. Usamos esto tanto para instalar como para desinstalar.
...
# Quiero crear una variable de donde está java en mi ordenador:
#version: java -version (en terminal)
#(en terminal) export JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk1.8.0_162.jdk/Contents/Home
...
#para ejecutar spark en local tenemos que ajustar los parámetros a la maquina (no es un cluster)
conf <- spark_config()
...
#conectando con spark. (sc es spark context)
...
#mis datos no estarían en R, estarían distribuidos por las maquiinas de spark
src_tbls(sc)

flights_tbl <- tbl(sc, 'flights')

#Podemos hacer las mismas funciones de dplyr.
...
#para copiar un df que tengamos en local a spark
...
#cogemos un 10% de los datos
model_data <- sdf_sample(model_data, 0.1)


# Partition the data into training and validation sets
#le decimos a spark que particione los datos en uno de entrenamiento y en otro de validacion.
model_partition <- model_data %>% 
  sdf_partition(train = 0.8, valid = 0.2, seed = 5555)

# Fit a linear model (que aplique ML, una regresionlineal )
